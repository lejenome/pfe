...
plusieurs tâches pour mieux atteindre le but à la fin de
l'itération.
...
Ce rôle était assigné à membre de l'équipe \textquote{City Watch}.
...
lignes colorées représentent nos tâches du binôme. Les colonnes du tableau sont:
...
\begin{description}
    ...
    \item [Implémentation] La pourcentage de l'implémentation du tâche de 0\%
        ...
        pourcentage de l'implémentation à chaque jour pendant un réunion de 20 minutes
        nommé \textquote{Daily Scrum} pendant le, on discute l'évolution du
...
Pour montrer l'avancement durant une itération donnée, le
Burndown Chart (graphique d'avancement) est tracé. Il s'agit d'un graphique simple qui
montre le nombre d'heures restant à effectuer pour finir les tâches d'itération
...
d'utilisations que nous allons les implémenter pendant les 3 premières itérations.
La définition des ces cas d'utilisation suit la forme suivante:
...
Pendant l'itération 0, nous avons fait un recherche sur les choix techniques à fin de
...
Nous avons choisi de supporter le système Android comme notre première plateforme. Ce
...
de développement dont le serveur supporte seulement PHP, nous avons choisi le langage
...
Pour le développement du front-end, nous avons choisi d'utiliser les technologies Web modernes qui nous permettre de
...
\begin{description}
    \item [HTML 5.1] Nous avons utilisé les tags sémantique, les nouveaux types et
        attributs de validation du formulaire et les nouveautés
...
L'utilisation de ces dernières mise à jours des standards du web a causé le support
de l'application web que par seulement les dernières versions des
