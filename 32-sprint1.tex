...
Concernant l'application le scénario est:
\begin{enumerate}
 \item Application Android locale installée --> interface contient deux boutons Start/Stop
 \item Lancer application en mode enligne (GPS + internet activités) sinon en mode
 hors ligne (GPS + internet désactivés) --> bouton 'Strat' activé et bouton 'Stop' désactivé
 \item Appuis sur la bouton 'Start' -->  bouton 'Strat' désactivé et bouton 'Stop' activé avec un message d'information
 \item En cas de rupture de la connexion internet --> retourne vers le cas d'utilisation numéro 2
 \item Appuis sur la bouton 'Stop' --> quitté l'application
\end{enumerate}

Concernant le web le scénario est:
\begin{enumerate}
 \item Lancer un navigateur(Chrome/Fierfox) + URL(serveur distant)
 \item Voir la carte (centré à Sfax )
 \item Un marqueur affiché sur la carte (position du marqueur la plus récente)
 \item La taille de marqueur reste constante lors du zoom
 \item L'icône du marqueur doit être personnalisé
 \item Sans faire d'action la position du marqueur change automatiquement pour montrer la dernier position de l'individu
 \item le curseur sur le marqueur un fenêtre(pop up) s'affiche --> montrant l'identifiant de la périphérique + longitude +l'altitude
 \item L'icône de l'individu change sa couleur selon leur mode (enligne/hors ligne)
 \item En cas de plusieurs utilisateur --> afficher un marqueur pour chacun
\end{enumerate}
...
\item L'application détecte la position à travers le capteur GPS du smartphone.
...
\begin{table}[!h]
 ...
 \begin{tabular}{| p{3cm} | l |}
  ...
  Acteur & Utilisateur \\ \hline
  Pré condition & Position déja enregistrée \\ \hline
...
\end{figure}
L'application finale qu'on a aborder dans cette itération composé de trois bouton 'Start', 'Stop' et 'Close'.


Le click sur le bouton 'Start' active la délectation continuelle de la position
qui nécessite la disponibilité de l'internet et du GPS.

Le click sur le bouton 'Stop' désactive la détection de la position.
