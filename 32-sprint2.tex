...
L'objectif de cette itération vise trois environnements.Nous allons ajouté une nouvelle vision paraport à 
l'itération précédentes.

Nous représentons par:

\begin{itemize}
 \item \textbf{Smart phone :} Ajouter des nouvelles fonctionnalités comme :
	\begin{itemize}
	 \item Capter l'embouteillages 
	 \item Détection secousses 
	 \item Déclaration les ralentisseurs
	\end{itemize}
\item \textbf{Dashboard :} Élaborer notre maps par :
	\begin{itemize}
	 \item Des nouvelles marqueur spécifié à chaque fonctionnalité ajouter.
	 \item Une légende pour le filtrage des marqueurs. 
	\end{itemize}
\item \textbf{Rapport :} Créer une page rapport qui permet à l'utilisateur de:
	\begin{itemize}
	 \item  Faire une description sur le type du rapport(violence, accident, déchet...) 
	 \item  Charger une image 
	 \item  Déclarer la position automatiquement  ou manuellement
	\end{itemize}
\end{itemize}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[draft,width=0.3\textwidth]{foo}
...
l'application.les données saisies dans le formulaire d'inscription (nom d'utilisation,mot de passe) seront
envoyées et stockées en toute sécurité et confidentialité .Lorsqu'un utilisateur tend à se connecter ,il 
tape son mot de passe déjà saisies dans le formulaire d'inscription,alors une vérification avec le serveur 
doit avoir lieu.

NB:Pour chaque nom d'utilisateur ajouté, le serveur vérifie l'unicité de ce dernier.\\
\begin{figure}[htbp]
    \begin{subfigure}{.5\textwidth}
    \centering
  \centering
